    ...
    const interval = setInterval(async () => {
      ...
      try {
        ...
        const [p, s, c, b, r, sl, queuedSl, u, au, invs, pr, tr, exr, adr] = await Promise.allSettled([
          ...
          section('sections.retail') && allow('pages.retail.purchases', ['Admin','Manager','Inventory Staff','Director'], ['approve_purchases','approve_retail_director','approve_retail_manager','view_purchases','see_purchases']) ? purchasesApi.listRequests({ status: 'pending_director', limit: 200 }) : Promise.resolve([]),
          section('sections.retail') && allow('pages.retail.transfers', ['Admin','Manager','Inventory Staff','Director'], ['approve_transfers','approve_retail_director','approve_retail_manager','view_transfers','see_transfers']) ? transfersApi.listRequests({ status: 'pending_director', limit: 200 }) : Promise.resolve([]),
          ...
          section('sections.retail') && allow('pages.retail.adjustments', ['Admin','Manager','Director'], ['approve_adjustments','approve_retail_director','approve_retail_manager']) ? adjustmentsApi.listRequests({ status: 'pending_director', limit: 200 }) : Promise.resolve([])
        ]);
      ...
      <LanguageProvider>
        ...
        <Routes>
          ...
          <Route element={<ProtectedRoute><Layout bootstrapLoading={!dataBootstrapReady} /></ProtectedRoute>}>
            ...
            <Route path="/warehouse-approvals" element={<ProtectedRoute feature="pages.warehouse.approvals" roles={['Admin','Manager','Director','SuperAdmin']} grant={['view_warehouse_approvals','approve_warehouse_director','approve_warehouse_manager']}><WarehouseApprovalsPage /></ProtectedRoute>} />
