...
export async function refreshAllData(dispatch, getState) {
  ...
  const canLoadRefunds = (
    ...
    || (isFeatureEnabled(settings, 'modules.approvalsCenter') && allow('modules.approvalsCenter', ['Admin','Manager','Director'], ['view_approvals','approve_retail_director','approve_retail_manager','approve_distribution_director','approve_distribution_manager','approve_warehouse_director','approve_warehouse_manager']))
    || (isFeatureEnabled(settings, 'pages.warehouse.approvals') && allow('pages.warehouse.approvals', ['Admin','Manager','Director'], ['view_warehouse_approvals','approve_warehouse_director','approve_warehouse_manager']))
